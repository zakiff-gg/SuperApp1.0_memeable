...
object PrefsHelper {
    ...
    private const val PREF_MEME_MODE = "meme_mode_pref"
    ...
    // ---------------- Preferensi Mode Meme ----------------
    // Toggle iseng: kalau aktif, suara & tampilan hasil tap absen diganti versi lucu/meme.
    // Default OFF -- harus diaktifkan manual lewat toggle di Standby Screen.

    fun setMemeMode(context: Context, aktif: Boolean) {
        val prefs = context.getSharedPreferences(AppConstants.PREFS_NAME, Context.MODE_PRIVATE)
        prefs.edit().putBoolean(PREF_MEME_MODE, aktif).apply()
    }

    fun isMemeMode(context: Context): Boolean {
        val prefs = context.getSharedPreferences(AppConstants.PREFS_NAME, Context.MODE_PRIVATE)
        return prefs.getBoolean(PREF_MEME_MODE, false)
    }
}
